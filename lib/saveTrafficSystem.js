...
class SafeTrafficSystem extends Contract {
  ...
  async RequestDrivingLicense(ctx, login, licenseNumber, expiryDate, category) {
    try {
  ...
  async ApproveDrivingLicenseRequest(
    ...
    try {
      const officerJSON = await ctx.stub.getState(officerLogin);
      if (!officerJSON || officerJSON.length === 0) {
        throw new Error(`Должностное лицо с Login ${officerLogin} не найдено`);
      }
      ...
      const newLicense = {
        ...
        ExpiryDate: driver.Requests[requestIndex].expiryDate,
