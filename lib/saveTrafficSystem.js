...
class SafeTrafficSystem extends Contract {
  ...
  async ApproveRegisterVehicle(ctx, login, recipientLogin, requestIndex) {
    const officerJSON = await ctx.stub.getState(login); // Получаем данные должностного лица по ID
    if (!officerJSON || officerJSON.length === 0) {
      throw new Error(`Должностное лицо с Login ${login} не найдено`); // Если должностное лицо не найдено, выбрасываем ошибку
    }
    ...
    if (
      ...
      throw new Error(`Запрос на регистрацию транспортного средства не найден`); // Если запрос не найден, выбрасываем ошибку
    }
    ...
    return `Запрос на регистрацию транспортного средства для водителя ${driver.FullName} утвержден.`; // Возвращаем сообщение об успешном утверждении
  }
